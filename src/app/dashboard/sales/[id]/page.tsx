    ...
    <div className="space-y-8 pb-20">
      <div className="flex justify-between items-center">
        <div>
          ...
          <h1 className="text-3xl font-black tracking-tight">Factura #{sale.id.slice(0, 8).toUpperCase()}</h1>
      ...
      <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div>
            ...
            {sale.customer_name ? (
              <div className="space-y-0.5">
                <p className="font-black text-slate-900 text-lg uppercase tracking-tight">{sale.customer_name}</p>
                {sale.customer_nit && <p className="text-sm font-medium text-slate-500 uppercase tracking-tight">{sale.customer_nit}</p>}
                {sale.customer_address && <p className="text-sm font-medium text-slate-500">{sale.customer_address}</p>}
                {sale.customer_phone && <p className="text-sm font-medium text-slate-500">{sale.customer_phone}</p>}
              </div>
            ) : (
              <p className="font-black text-slate-900 text-lg">Cliente no registrado</p>
            )}
          </div>
          <div className="text-right">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Vendedor</p>
            <p className="font-black text-slate-900 text-lg uppercase tracking-tight">{sellerName || '—'}</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-5 text-xs font-black text-slate-400 uppercase tracking-widest text-center">Cant.</th>
                <th className="px-6 py-5 text-xs font-black text-slate-400 uppercase tracking-widest text-right">Precio</th>
                <th className="px-6 py-5 text-xs font-black text-slate-400 uppercase tracking-widest text-right">IVA</th>
                ...
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-slate-900">{item.product?.name || 'Producto eliminado'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5 text-sm text-slate-500 font-medium">{item.product?.brand || '—'}</td>
                  <td className="px-6 py-5 text-center text-sm font-bold text-slate-700">{item.quantity}</td>
                  <td className="px-6 py-5 text-right text-sm font-bold text-slate-700 tabular-nums">{formatCurrency(item.unit_base_price)}</td>
                  <td className="px-6 py-5 text-right text-sm font-bold text-slate-500 tabular-nums">{formatCurrency(item.tax_amount)}</td>
                  <td className="px-8 py-5 text-right font-black text-slate-900 tabular-nums">{formatCurrency(item.subtotal)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {sale.notes && (
          <div className="px-10 py-6 border-t border-slate-100 bg-white">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Notas</p>
            <p className="text-sm font-medium text-slate-600 whitespace-pre-wrap">{sale.notes}</p>
          </div>
        )}
        ...
        <div className="p-10 bg-slate-50/30 flex justify-end border-t border-slate-100">
          <div className="w-full md:w-80 space-y-3">
            ...
            <div className="pt-6 border-t border-slate-200 flex justify-between items-end">
              ...
              <span className="text-4xl font-black text-slate-900 tracking-tighter leading-none">{formatCurrency(sale.total_amount)}</span>
            </div>
          </div>
        </div>
      </div>
